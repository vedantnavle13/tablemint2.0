...
// ── WalkInBillModal ────────────────────────────────────────────────────────────
function WalkInBillModal({ order, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 3000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}
      ...
      <div style={{ background: "#fff", borderRadius: 20, padding: 36, maxWidth: 480, width: "100%", boxShadow: "0 24px 60px rgba(0,0,0,0.3)", maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 900, color: C.text }}>TableMint</div>
          <div style={{ fontSize: 12, color: C.textMuted, marginTop: 4 }}>Walk-in Order Bill</div>
        </div>
        <div style={{ borderTop: `2px dashed ${C.border}`, borderBottom: `2px dashed ${C.border}`, padding: "14px 0", marginBottom: 18 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 12, color: C.textMuted, fontWeight: 600 }}>Walk-in ID</span>
            <span style={{ fontFamily: "monospace", fontSize: 13, fontWeight: 700, color: C.amber }}>{order.walkInId}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 12, color: C.textMuted, fontWeight: 600 }}>Transaction</span>
            <span style={{ fontFamily: "monospace", fontSize: 12, color: C.textMid }}>{order.transactionId}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 12, color: C.textMuted, fontWeight: 600 }}>Table</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{order.tableNumber}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, color: C.textMuted, fontWeight: 600 }}>Customer</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{order.customerName}</span>
          </div>
        </div>
        {(order.items || []).map((item, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: `1px solid ${C.border}` }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>₹{(item.price * item.quantity).toFixed(2)}</span>
          </div>
        ))}
        <div style={{ marginTop: 16, padding: "12px 0", borderTop: `2px solid ${C.border}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 13, color: C.textMid }}>Subtotal</span>
            <span style={{ fontSize: 13, color: C.text }}>₹{(order.subtotal || 0).toFixed(2)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 13, color: C.textMid }}>GST (18%)</span>
            <span style={{ fontSize: 13, color: C.text }}>₹{(order.tax || 0).toFixed(2)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
            <span style={{ fontSize: 16, fontWeight: 900, color: C.text }}>Grand Total</span>
            <span style={{ fontSize: 18, fontWeight: 900, color: C.amber }}>₹{(order.grandTotal || 0).toFixed(2)}</span>
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
          <button onClick={handlePrint}
            style={{ flex: 1, padding: "12px", background: C.amber, border: "none", borderRadius: 10, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
            🖨️ Print Bill
          </button>
          <button onClick={onClose}
            style={{ padding: "12px 20px", background: "transparent", border: `1.5px solid ${C.border}`, borderRadius: 10, color: C.textMid, fontWeight: 600, fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
            Close
          </button>
        </div>
      </div>
...
function ReservationsTab() {
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { if (subTab === "walkins") loadWalkIns(); }, [subTab, loadWalkIns]);
  ...
  const handleGenerateBill = async (order) => {
    setWiBilling(order._id);
    try {
      const res = await apiCall(`/admin/walk-in/${order._id}/bill`, "POST");
      const updated = res.data.walkIn;
      setWalkIns(prev => prev.map(w => w._id === updated._id ? updated : w));
      setWiBill(updated);
    } catch (e) { alert(e.response?.data?.message || "Failed to generate bill."); }
    finally { setWiBilling(null); }
  };

  const handleMarkPaid = async (order) => {
    if (!window.confirm(`Mark ${order.walkInId} as PAID?`)) return;
    setWiPaying(order._id);
    try {
      const res = await apiCall(`/admin/walk-in/${order._id}/pay`, "PATCH");
      setWalkIns(prev => prev.map(w => w._id === order._id ? { ...w, paymentStatus: "paid", paidAt: res.data.paidAt } : w));
    } catch (e) { alert(e.response?.data?.message || "Failed to mark paid."); }
    finally { setWiPaying(null); }
  };
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        ...
        {/* Sub-tab switcher */}
        <div style={{ display: "flex", background: C.bgSoft, borderRadius: 12, padding: 4, gap: 4 }}>
          {[("online"), ("walkins")].map(t => (
            <button key={t} onClick={() => setSubTab(t)} style={{
              padding: "7px 18px", borderRadius: 9, border: "none",
              background: subTab === t ? C.amber : "transparent",
              color: subTab === t ? "#fff" : C.textMid,
              fontWeight: 700, fontSize: 13, cursor: "pointer",
              fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s",
            }}>{t === "online" ? "💳 Online" : "🚶 Walk-ins"}</button>
          ))}
        </div>
        {subTab === "online" && (
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search customer, ID…"
            style={{ padding: "9px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 13, outline: "none", minWidth: 220, fontFamily: "'DM Sans', sans-serif", color: C.text }}
            onFocus={e => e.target.style.borderColor = C.amber} onBlur={e => e.target.style.borderColor = C.border} />
        )}
      </div>
                      ...
                      <td style={{ padding: "13px 14px" }}>
                        ...
                        {r.status === "completed" && r.billGeneratedAt && (
                          <div style={{ fontSize: 12, color: C.green, fontWeight: 700, marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 10, color: C.textMuted, fontWeight: 400, marginLeft: 4 }}>({r.paymentStatus || "unpaid"})</span>
                          </div>
                        )}
                        {r.customerMessage && (
      ...
      {subTab === "walkins" && (
        <>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <p style={{ fontSize: 13, color: C.textMuted }}>{walkIns.length} walk-in order{walkIns.length !== 1 ? "s" : ""} for your restaurant</p>
            <button onClick={loadWalkIns} style={{ fontSize: 12, color: C.amber, background: "transparent", border: `1px solid ${C.amber}40`, borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}>↻ Refresh</button>
          </div>
          {wiLoading ? <Spinner /> : walkIns.length === 0 ? <Empty icon="🚶" title="No walk-in orders yet." sub="Walk-in orders created via the Captain App will appear here." /> : (
            <div style={{ background: C.bgCard, borderRadius: 20, border: `1px solid ${C.border}`, overflowX: "auto" }}>
              <table style={{ borderCollapse: "collapse", width: "100%" }}>
                <thead><tr style={{ background: C.bgSoft, borderBottom: `2px solid ${C.border}` }}>
                  {["WALK-IN ID", "TRANSACTION", "CUSTOMER / TABLE", "ITEMS", "TOTAL", "STATUS", "ACTIONS"].map(h => (
                    <th key={h} style={{ padding: "13px 14px", textAlign: "left", fontSize: 11, fontWeight: 700, color: C.textMuted, letterSpacing: 1, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {walkIns.map(w => (
                    <tr key={w._id} style={{ borderBottom: `1px solid ${C.border}`, transition: "background 0.15s" }}
                      onMouseEnter={e => e.currentTarget.style.background = C.bgSoft}
                      onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                      <td style={{ padding: "13px 14px" }}>
                        <span style={{ fontFamily: "monospace", fontWeight: 800, fontSize: 14, color: C.amber }}>{w.walkInId}</span>
                        <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>{new Date(w.createdAt).toLocaleString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</div>
                      </td>
                      <td style={{ padding: "13px 14px" }}>
                        <span style={{ fontFamily: "monospace", fontSize: 12, color: C.textMid }}>{w.transactionId}</span>
                      </td>
                      <td style={{ padding: "13px 14px" }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{w.customerName}</div>
                        <div style={{ fontSize: 12, color: C.textMuted }}>Table: {w.tableNumber}</div>
                      </td>
                      <td style={{ padding: "13px 14px" }}>
                        {(w.items || []).slice(0, 2).map((it, i) => (
                          <div key={i} style={{ fontSize: 12, color: C.textMid }}>{it.name} ×{it.quantity}</div>
                        ))}
                        {(w.items || []).length > 2 && <div style={{ fontSize: 11, color: C.textMuted }}>+{w.items.length - 2} more</div>}
                      </td>
                      <td style={{ padding: "13px 14px" }}>
                        <div style={{ fontSize: 15, fontWeight: 800, color: C.text }}>₹{(w.grandTotal || 0).toFixed(2)}</div>
                        <div style={{ fontSize: 11, color: C.textMuted }}>incl. ₹{(w.tax || 0).toFixed(2)} GST</div>
                      </td>
                      <td style={{ padding: "13px 14px" }}>
                        {w.paymentStatus === "paid" ? (
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 700, color: C.green, background: C.green + "15", border: `1px solid ${C.green}40`, padding: "4px 10px", borderRadius: 20 }}>
                            ✓ PAID
                          </span>
                        ) : (
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 700, color: C.amber, background: C.amberSoft, border: `1px solid ${C.amber}40`, padding: "4px 10px", borderRadius: 20 }}>
                            PENDING
                          </span>
                        )}
                        {w.billGeneratedAt && <div style={{ fontSize: 11, color: C.textMuted, marginTop: 3 }}>🧾 Billed</div>}
                      </td>
                      <td style={{ padding: "13px 14px", minWidth: 140 }}>
                        <button onClick={() => handleGenerateBill(w)} disabled={wiBilling === w._id}
                          style={{ display: "block", width: "100%", marginBottom: 6, padding: "6px 10px", fontSize: 11, fontWeight: 700, borderRadius: 8, border: `1.5px solid ${C.amber}`, background: C.amberSoft, color: C.amber, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                          {wiBilling === w._id ? "Generating…" : "🖨️ Print Bill"}
                        </button>
                        {w.paymentStatus !== "paid" && (
                          <button onClick={() => handleMarkPaid(w)} disabled={wiPaying === w._id}
                            style={{ display: "block", width: "100%", padding: "6px 10px", fontSize: 11, fontWeight: 700, borderRadius: 8, border: `1.5px solid ${C.green}`, background: C.green + "15", color: C.green, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                            {wiPaying === w._id ? "Updating…" : "✔ Mark Paid"}
                          </button>
                        )}
                      </td>
                    </tr>
